...
export const getPublicTable = async (qrCodeToken: string) => {
  return await axiosBasic.get<ApiResponse<PublicTableResponse>>(
...
export const getPublicSessionMenu = async (sessionCode: string, query: SessionMenuQuery) => {
  return await axiosBasic.get<ApiResponse<SessionMenuResponse>>(
...
export const getPublicBranchCategories = async (branchId: string) => {
  return await axiosBasic.get<ApiResponse<PublicCategoryResponse[]>>(
    `/api/public/branches/${branchId}/categories`
  );
};
