...
import {
  usePendingWaiterOrdersQuery,
  useReadyToServeItemsQuery,
...
export const MyBranchOrdersPage = ({ branchId }: Props) => {
  ...
  const readyItemIds = useMemo(
    () =>
      readyGroups.flatMap((table) =>
        table.orders.flatMap((order) => order.items.map((item) => item.orderItemId))
      ),
    [readyGroups]
  );
      ...
      stats={
        <>
          <PortalStatCard
            ...
            helper="Đơn khách vừa gửi"
          />
          <PortalStatCard
            label="Bàn chờ phục vụ"
            value={String(readyGroups.length)}
            helper="Có món đã sẵn sàng"
          />
          <PortalStatCard
            label="Phần món sẵn sàng"
            value={String(readyPortions)}
            helper="Món cần mang ra bàn"
          />
          <PortalStatCard
            label="Đã chọn"
            value={String(selectedReadyIds.length)}
            helper="Món sẽ đánh dấu đã phục vụ"
          />
        </>
      }
    >
      ...
      {queryError ? (
        <div className="border-destructive/40 bg-destructive/10 text-destructive rounded-xl border p-6">
          ...
          <p className="mt-2 text-sm">
            {getApiErrorMessage(queryError, "Vui lòng thử lại hàng đợi của chi nhánh này.")}
          </p>
        </div>
      ) : null}
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Kiểm tra món và ghi chú trước khi gửi xuống bếp.
            </p>
          </div>
        ...
        {!pendingQuery.isLoading && pendingOrders.length === 0 ? (
          <div className="bg-card border-border/60 rounded-xl border p-8 text-center shadow-sm">
            ...
            <p className="text-muted-foreground mt-3 text-sm">
              Chưa có đơn khách nào chờ xác nhận.
            </p>
          </div>
        ) : null}
      ...
      <section className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Chọn đúng món đã mang ra bàn rồi đánh dấu đã phục vụ.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={toggleAllReady} disabled={readyItemIds.length === 0}>
              {selectedReadyIds.length === readyItemIds.length && readyItemIds.length > 0
                ? "Bỏ chọn"
                : "Chọn tất cả"}
            </Button>
            <Button
              ...
              disabled={!selectedReadyIds.length || serveMutation.isPending}
            >
        ...
        {!readyQuery.isLoading && readyGroups.length === 0 ? (
          <div className="bg-card border-border/60 rounded-xl border p-8 text-center shadow-sm">
            ...
            <p className="text-muted-foreground mt-3 text-sm">
              Hiện chưa có món nào sẵn sàng phục vụ.
            </p>
          </div>
        ) : null}
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {readyGroups.map((table) => (
            <article
              key={table.tableId ?? table.tableNumber ?? "unassigned"}
              className="bg-card border-border/60 rounded-xl border p-5 shadow-sm"
            >
                        ...
                        >
                          ...
                          <span className="min-w-0 flex-1 text-sm">
                            ...
                            <span className="text-muted-foreground block text-xs">
                              Sẵn sàng lúc {formatDateTime(item.readyAt)}
                            </span>
                          </span>
                        </label>
                      ))}
